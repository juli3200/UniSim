...
fn main() {
   let settings: Settings = settings!("experiments/toxic/toxic.json");
    ...
    // food source
    world.add_ligand_source(vec![50.0, 50.0], 10000.0, 2).expect("Failed to add ligand source");

    // toxin source
    world.add_ligand_source(vec![60.0, 60.0], 10000.0, 3).expect("Failed to add ligand source");
